    ...
    public class FieldOfViewCheck : INPCTransition
    {
        ...
        private bool IsPlayerInFoV()
        {
            ...
            if (Vector2.Angle(forwardVector2, directionToTarget.normalized) < _angle / 2)
            {
                ...
                _canSeePlayer = Physics.Raycast(_transform.position, _follower.Target.position - _transform.position + Vector3.up, distanceToTarget + 1,
                    _playerMask);
        ...
        public void DrawGizmos()
        {
            ...
            Gizmos.DrawLine(_transform.position + Vector3.up, _transform.position + viewAngle01 * sphereCollider.radius  + Vector3.up);
            Gizmos.DrawLine(_transform.position + Vector3.up, _transform.position + viewAngle02 * sphereCollider.radius  + Vector3.up);

            if (_canSeePlayer && _follower.Target != null)
            {
                Gizmos.color = Color.green;
                Gizmos.DrawLine(_transform.transform.position, _follower.Target.position);
